...
import { Wrench, Filter, MapPin, ChevronLeft, ChevronRight, Building2, AlertCircle, Clock, Car, Search, X, ChevronDown, ChevronUp } from "lucide-react";
...
const MAINTENANCE_STATUSES: FleetStatus[] = [
  'Em Oficina - Externo',
  'Em Oficina - Rentals',
  'Em Oficina - Trois',
  'PRA Reboque',
...
export default function MaintenanceFleet() {
  // Filtros principais
  ...
  // Filtrar apenas veículos em manutenção
  const maintenanceVehicles = useMemo(() => {
    return fleetVehicles.filter(v => MAINTENANCE_STATUSES.includes(v.status));
  }, []);

  // Obter valores únicos para filtros
  const uniqueUFs = useMemo(() => 
    Array.from(new Set(maintenanceVehicles.map(v => v.uf).filter(Boolean))).sort(),
    [maintenanceVehicles]
  );

  const uniqueGestores = useMemo(() => 
    Array.from(new Set(maintenanceVehicles.map(v => v.gestor).filter(Boolean))).sort(),
    [maintenanceVehicles]
  );

  const uniqueBases = useMemo(() => 
    Array.from(new Set(maintenanceVehicles.map(v => v.base).filter(Boolean))).sort(),
    [maintenanceVehicles]
  );

  const uniqueStatuses = useMemo(() => 
    Array.from(new Set(maintenanceVehicles.map(v => v.status))).sort(),
    [maintenanceVehicles]
  );

  // Filtrar veículos baseado nos filtros principais
  const filteredByMainFilters = useMemo(() => {
    return maintenanceVehicles.filter(vehicle => {
  ...
  }, [maintenanceVehicles, selectedUFs, selectedGestores, selectedBases, selectedStatuses]);
  ...
  // Dados para gráfico de pizza - Gestores
  const gestoresPieData = useMemo(() => {
  ...
  const getStatusIcon = (status: string) => {
    const icons: { [key: string]: React.ReactNode } = {
      'Em Oficina - Externo': <Building2 className="h-4 w-4" />,
      'Em Oficina - Rentals': <Building2 className="h-4 w-4" />,
      'Em Oficina - Trois': <Building2 className="h-4 w-4" />,
      'PRA Reboque': <AlertCircle className="h-4 w-4" />,
      'Aguardando Oficina': <Clock className="h-4 w-4" />,
    };
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <CardContent>
              <ResponsiveContainer width="100%" height={400}>
                <BarChart data={gestoresPieData} layout="vertical">
                  <CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--border))" />
                  <XAxis type="number" tick={{ fill: 'hsl(var(--muted-foreground))' }} />
                  <YAxis 
                    type="category" 
            ...
            <Card className="border-primary/20">
              <CardHeader>
                <div className="flex items-center justify-between">
                  <CardTitle className="text-lg">Veículos em Manutenção</CardTitle>
                  <Button
                    variant="ghost"
                    size="sm"
